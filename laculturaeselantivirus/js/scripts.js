var s = (sketch) => {
  ...
  sketch.draw = () => {
    ...
    for (let i = 0; i < spectrum.length; i += 6) {
      sketch.vertex(
        sketch.map(i, 0, spectrum.length, 0, sketch.width),
        sketch.map(spectrum[i], 0, 255, 0, sketch.height / 2)
      );
    }
    sketch.endShape();
    ...
    for (let i = spectrum.length; i > 0; i -= 6) {
      sketch.vertex(
        sketch.map(i, 0, spectrum.length, sketch.width, 0),
        sketch.map(spectrum[i], 0, 255, sketch.height, sketch.height / 2)
      );
    }
    sketch.endShape();
  };
...
function updateTimeText(current, total) {
  ...
  timeTxt.innerHTML =
    this.secondsTimeSpanToHMS(current) + '/' + this.secondsTimeSpanToHMS(total);
}
